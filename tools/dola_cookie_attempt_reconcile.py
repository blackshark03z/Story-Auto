"""Read-only exact-attempt lookup after one ambiguous Dola submit.

This does not submit, retry, adopt a receipt, or conclude that an absent
recent-conversation match proves no provider-side effect.
"""
...
from tools.dola_cookie_profile_canary import ACCOUNT_ID, SLOT_ID, _manifest
...
def _matches(messages: object, attempt_id: str, conversation_id: str) -> tuple[bool, bool]:
    if not isinstance(messages, list):
        return False, False
    inputs = [message for message in messages if isinstance(message, dict)
              and message.get("local_message_id") == attempt_id
              and message.get("conversation_id") in {None, "", conversation_id}
              and any(isinstance(block, dict) and block.get("block_type") == 10000
                      for block in _content(message))
              and not any(isinstance(block, dict) and block.get("block_type") == 2074
                          for block in _content(message))]
    input_ids = {message.get("message_id") for message in inputs
                 if isinstance(message.get("message_id"), str)}
    linked_video = any(
        isinstance(message, dict)
        and message.get("bot_reply_message_id") in input_ids
        and any(isinstance(block, dict) and block.get("block_type") == 2074
                for block in _content(message))
        for message in messages
    )
    return bool(inputs), linked_video


def _content(message: dict) -> list:
...
def main() -> int:
    ...
    if (attempt.get("account_id") != ACCOUNT_ID
            or attempt.get("status") != "AMBIGUOUS"
            or attempt.get("submit_attempts") != 1
            or attempt.get("provider_task_id")):
        raise SystemExit("EXACT_AMBIGUOUS_ATTEMPT_REQUIRED")
    attempt_id = attempt["attempt_id"]
    result: dict[str, object] = {
        "schema": "story-auto-dola-ambiguous-attempt-read/1",
        "scope": "read-only recent/single chain lookup for one persisted local attempt",
        "generation_submits": 0, "cookie_values_recorded": False,
        "provider_ids_recorded": False, "message_content_recorded": False,
        "attempt_id": attempt_id, "saved_status": "AMBIGUOUS",
    }
    try:
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                for identifier in ids:
                    ...
                    has_input, has_linked_video = _matches(messages, attempt_id, identifier)
                    if has_input:
                        matches.append((identifier, has_linked_video))
                ...
                if len(matches) == 1:
                    try:
                        status = DolaCookieClient(cookie).poll(
                            matches[0][0], client_request_id=attempt_id)["status"]
    ...
    return 0 if result["conclusion"] in {"EXACT_RECENT_MATCH", "NO_MATCH_IN_RECENT_WINDOW_NOT_NO_EFFECT_PROOF"} else 2
