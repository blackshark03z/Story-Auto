...
function openingBuilderSurface(snapshot) {
  ...
  const slots = (opening.slots || []).map(slot => {
    ...
    const source = slot.source_asset ? `<small>Imported ${esc(slot.source_asset.original_filename || 'clip')} Ã‚Â· ${Number(slot.source_asset.duration_seconds || 0).toFixed(2)}s${slot.source_asset.had_audio ? ' Ã‚Â· embedded audio stripped' : ''}</small>` : '<small>No clip imported yet.</small>';
    return `<article class="choice"><div class="surface-head"><div><strong>${esc(slot.slot_id)} Ã‚Â· ${esc(slot.start)}Ã¢â‚¬â€œ${esc(slot.end)}s</strong><small>${esc(slot.purpose)}</small></div><span class="status-chip ${ready ? 'success' : 'attention'}">${ready ? 'READY' : 'MISSING'}</span></div><div class="technical opening-prompt">${esc(slot.prompt)}</div><div class="button-row"><button data-opening-copy="${esc(slot.slot_id)}" type="button">Copy prompt</button><label class="button">${ready ? 'Replace clip' : 'Import clip'}<input data-opening-import="${esc(slot.slot_id)}" type="file" accept="video/*" hidden></label></div>${source}${preview}</article>`;
  }).join('');
