...
function projectCard(project) {
  ...
  return `<article class="project-card ${project.attention?.length ? 'is-attention' : ''}">
    <div class="card-top"><div><h3>${esc(project.title)}</h3><p class="meta">${esc(formatUpdated(project.updated_at))} Ã‚Â· ${esc(humanMode(project.render_mode))}${project.ambient_style_label ? ` Ã‚Â· ${esc(project.ambient_style_label)}` : ''}</p></div><span class="status-chip ${chipClass(project)}">${esc(project.user_status)}</span></div>
...
async function showHome(announce = false) {
  ...
  setNav('home');
  setHeader('YOUR VIDEOS','Home','<button class="button-primary" id="newVideoTop" type="button">Ã¯Â¼â€¹ New video</button>');
  $('#newVideoTop').addEventListener('click', openWizard);
  const view = $('#view');
  view.innerHTML = '<div class="empty-library"><p>Loading your videosÃ¢â‚¬Â¦</p></div>';
...
function timingControls(snapshot) {
  ...
  return `<section class="surface"><div class="surface-head"><div><h2>Audio and subtitle timing</h2><p>Audio=${esc(snapshot.narration_audio || 'MISSING')} Ã‚Â· SRT=${esc(srt)} Ã‚Â· TIMING SOURCE=${esc(timing)}</p></div></div><div class="choice-grid"><div class="choice"><strong>TTS = ${ttsSkipped ? 'SKIPPED' : 'REUSE/SKIP'}</strong><small>${timing === 'SRT' ? 'SRT is the canonical timing source; no TTS request will run.' : ttsSkipped ? 'Imported narration is canonical; no TTS request will run.' : 'Narration reuse is validated before downstream work.'}</small></div><div class="choice"><strong>Visual planning = ${planning}</strong><small>Visuals = ${snapshot.accepted_visuals ? 'REUSE' : 'GENERATE'} Ã‚Â· Render = ${snapshot.duration_seconds ? 'READY' : 'BLOCKED'}.</small></div></div></section>`;
}
...
function projectHeader(snapshot) {
  setHeader('PROJECT', snapshot.title, '<button class="button-quiet" id="backHome" type="button">Ã¢â€ Â Home</button>');
...
function fullVideoProviderSurface(snapshot) {
  ...
  const routing = provider.routing_enabled ? 'Production enabled' : 'Integration staged Ã¢â‚¬â€ production routing remains locked until Goal 54 UAT is accepted.';
...
function bindHybridBodyControls() {
  document.querySelectorAll('[data-hybrid-body-plan]').forEach(button => button.addEventListener('click', async () => {
    await runAction('plan_hybrid_body','Planning Hybrid Visual body from narrationÃ¢â‚¬Â¦');
  }));
  document.querySelectorAll('[data-hybrid-stock-resolve]').forEach(button => button.addEventListener('click', async () => {
    await runAction('resolve_hybrid_stock_slot','Finding and normalizing one relevant Pexels clipÃ¢â‚¬Â¦',{slot_id:button.dataset.hybridStockResolve});
  }));
  document.querySelectorAll('[data-hybrid-image-import]').forEach(input => input.addEventListener('change', async event => {
    ...
    try {
      ...
      await runAction('import_hybrid_body_image',`Binding ${input.dataset.hybridImageImport} imageÃ¢â‚¬Â¦`,{slot_id:input.dataset.hybridImageImport,imported_image,as_stock_fallback:input.dataset.stockFallback === 'true'});
    } catch (error) { toast(friendlyError(error).message,true); }
  }));
  document.querySelectorAll('[data-hybrid-preview-render]').forEach(button => button.addEventListener('click', async () => {
    await runAction('render_hybrid_preview','Rendering mixed Hybrid Visual preview with master audio/subtitles/waveformÃ¢â‚¬Â¦');
  }));
}
...
function bindOpeningBuilderControls() {
  document.querySelectorAll('[data-opening-prepare]').forEach(button => button.addEventListener('click', async () => {
    await runAction('prepare_opening_builder','Preparing exact opening prompts from the saved planÃ¢â‚¬Â¦');
  }));
  ...
  document.querySelectorAll('[data-opening-import]').forEach(input => input.addEventListener('change', async event => {
    ...
    try {
      ...
      await runAction('import_opening_clip',`Normalizing ${input.dataset.openingImport}Ã¢â‚¬Â¦`,{slot_id:input.dataset.openingImport,imported_video});
    } catch (error) { toast(friendlyError(error).message,true); }
  }));
}
...
function bindFullVideoProviderControls() {
  document.querySelectorAll('[data-elyum-review]').forEach(button => button.addEventListener('click', async () => {
    ...
    await runAction('review_elyum_preview', decision === 'ACCEPT' ? 'Recording preview acceptanceÃ¢â‚¬Â¦' : 'Recording preview rejectionÃ¢â‚¬Â¦', {request_id:button.dataset.requestId,decision,reason});
  }));
  document.querySelectorAll('[data-elyum-keep]').forEach(button => button.addEventListener('click', async () => {
    ...
    await runAction('keep_elyum_preview','Keeping and acquiring the clean outputÃ¢â‚¬Â¦',{request_id:button.dataset.requestId,confirm_spend:true});
  }));
  document.querySelectorAll('[data-elyum-kill]').forEach(button => button.addEventListener('click', async () => {
    ...
    await runAction('kill_elyum_preview','Killing the rejected provider resultÃ¢â‚¬Â¦',{request_id:button.dataset.requestId,confirm_kill:true,reason});
  }));
  document.querySelectorAll('[data-elyum-reacquire]').forEach(button => button.addEventListener('click', async () => {
    await runAction('keep_elyum_preview','Retrying clean output acquisition without another KeepÃ¢â‚¬Â¦',{request_id:button.dataset.requestId,confirm_spend:false});
  }));
  document.querySelectorAll('[data-elyum-replace]').forEach(button => button.addEventListener('click', async () => {
    ...
    await runAction('authorize_elyum_replacement','Authorizing one replacement attemptÃ¢â‚¬Â¦',{request_id:button.dataset.requestId,reason,confirm_replace:true,replacement_prompt:replacementPrompt});
  }));
}
...
function renderProject() {
  ...
  const activeText=visual.status === 'RUNNING' && visual.total_items ? `Creating visuals Ã¢â‚¬â€ ${visual.completed_items} of ${visual.total_items}` : (blocker?.human_message || recovery.human_message || 'Completed stages are saved. Continue resumes the canonical production path.');
  syncActivityPresentation();
  ...
  const primary=`<button class="button-primary" data-project-action="${esc(immediateAction.action)}" type="button" ${state.busy ? 'disabled' : ''}>${esc(backendProductionWorking(production) ? 'WorkingÃ¢â‚¬Â¦' : immediateAction.label)}</button>`;
  ...
  document.querySelectorAll('[data-rebind-flow]').forEach(button => button.addEventListener('click', async () => {
    ...
    await runAction('rebind_flow_project','Rebinding future Flow requestsÃ¢â‚¬Â¦', {explicit_owner_decision:true});
  }));
  $('#reviewPlan')?.addEventListener('click', showPlanReview);
  $('#reviewProject')?.addEventListener('click', showReview);
  $('#renderAgain')?.addEventListener('click', () => runAction('render_again','Rendering the final video againÃ¢â‚¬Â¦'));
  bindErrorActions();
...
function renderComplete(snapshot) {
  ...
  $('#openFolder').addEventListener('click', () => runAction('open_output','Opening the output folderÃ¢â‚¬Â¦'));
  $('#renderAgain')?.addEventListener('click', () => runAction('render_again','Rendering the final videoÃ¢â‚¬Â¦'));
  $('#renderAgain').addEventListener('click', () => runAction('render_again','Rendering the final video againÃ¢â‚¬Â¦'));
  bindDiagnosticsDisclosure();
}
...
async function handleProjectAction(action) {
  ...
  if (action === 'process' || action === 'run_to_final') return runAction('run_to_final','Continuing production until it needs your decisionÃ¢â‚¬Â¦');
  if (action === 'continue_production') return runAction('continue_production','Continuing production until it needs your decisionÃ¢â‚¬Â¦');
  if (action === 'validate_flow_connection') return runAction('validate_flow_connection','Validating the expected Flow projectÃ¢â‚¬Â¦');
  if (action === 'plan_visuals') return runAction('plan_visuals','Preparing the visual planÃ¢â‚¬Â¦');
  if (action === 'resume_generation') return runAction('resume_generation','Creating visuals. Completed work remains savedÃ¢â‚¬Â¦');
  if (action === 'render') return runAction('render','Rendering the final videoÃ¢â‚¬Â¦');
  if (action === 'open_flow_sign_in') return runAction('open_flow_sign_in','Opening the Story Auto Flow sign-in windowÃ¢â‚¬Â¦');
  if (action === 'open_flow_project') return runAction('open_flow_project','Opening this videoÃ¢â‚¬â„¢s Flow projectÃ¢â‚¬Â¦');
  if (action === 'ensure_flow_project') return runAction('ensure_flow_project','Finishing this videoÃ¢â‚¬â„¢s Flow project setupÃ¢â‚¬Â¦');
  if (action === 'recheck_flow_generation' || action === 'recheck_status') return runAction('recheck_flow_generation','Rechecking the exact Flow generationÃ¢â‚¬Â¦');
}
...
async function requestPause() {
  if (!state.project) return;
  const button = $('#pauseProject'); if (button) { button.disabled = true; button.textContent = 'PausingÃ¢â‚¬Â¦'; }
  try {
...
function friendlyError(error) {
  ...
  const known = {
    ...
    FLOW_PROJECT_MISMATCH: ['Flow project does not match','Story Auto could not confirm this videoÃ¢â‚¬â„¢s exact Flow project. Open the expected project and try again; no manual URL copy is required.','open_flow_project','Open expected Flow project'],
...
async function showContentEditor() {
  ...
  setHeader('PROJECT CONTENT',state.snapshot.title,'<button class="button-quiet" id="backProject" type="button">Ã¢â€ Â Project</button>');
...
async function showPlanReview() {
  ...
  setHeader('REVIEW PLAN',state.snapshot.title,'<button class="button-quiet" id="backProject" type="button">Ã¢â€ Â Project</button>');
  ...
  $('#approveCurrentPlan').addEventListener('click', async () => {
    setBusy(true,hasVisualPlan ? 'Approving the visual planÃ¢â‚¬Â¦' : 'Preparing the visual planÃ¢â‚¬Â¦');
    try {
...
async function showReview() {
  setBusy(true,'Opening reviewÃ¢â‚¬Â¦');
  try {
    ...
    setHeader('REVIEW',review.title,'<button class="button-quiet" id="backProject" type="button">Ã¢â€ Â Project</button>');
...
async function showSettings() {
  ...
  $('#view').innerHTML = '<div class="empty-library"><p>Loading settingsÃ¢â‚¬Â¦</p></div>';
  ...
  $('#saveByteplusKey')?.addEventListener('click', async () => {
    const key=$('#byteplusApiKey').value.trim();
    if (!key) { toast('Paste a BytePlus ModelArk API key first.',true); $('#byteplusApiKey').focus(); return; }
    try { await api('/api/settings/byteplus/save',{method:'POST',body:JSON.stringify({key})}); $('#byteplusApiKey').value=''; toast('BytePlus key saved securely.'); await showSettings(); }
    catch (error) { toast(friendlyError(error).message,true); }
  });
  ...
  $('#savePexelsKey')?.addEventListener('click', async () => {
    const key=$('#pexelsApiKey').value.trim();
    if (!key) { toast('Paste a Pexels API key first.',true); $('#pexelsApiKey').focus(); return; }
    try { await api('/api/settings/pexels/save',{method:'POST',body:JSON.stringify({key})}); $('#pexelsApiKey').value=''; toast('Pexels key saved securely.'); await showSettings(); }
    catch (error) { toast(friendlyError(error).message,true); }
  });
  ...
  $('#clearPexelsKey')?.addEventListener('click', async () => {
    try { await api('/api/settings/pexels/clear',{method:'POST',body:'{}'}); toast('Saved Pexels key removed.'); await showSettings(); }
    catch (error) { toast(friendlyError(error).message,true); }
  });
...
async function showDiagnostics(projectId) {
  if (!projectId) return;
  setHeader('SETTINGS / ADVANCED','Diagnostics','<button class="button-quiet" id="backSettings" type="button">Ã¢â€ Â Settings</button>');
  $('#view').innerHTML = '<div class="empty-library"><p>Loading diagnosticsÃ¢â‚¬Â¦</p></div>';
  try {
...
function sourceStatusMarkup(wizard) {
  ...
  const mark = component => component?.status === 'READY' ? 'Ã¢Å“â€œ' : component?.status === 'BLOCKED' ? 'Ã¢Å“â€¢' : 'Ã¢â‚¬â€œ';
...
async function advanceWizard() {
  ...
  const button = $('#wizardNext'); wizard.creating = true; button.disabled = true; button.textContent = 'CreatingÃ¢â‚¬Â¦';
  try {
    ...
    closeWizard(); toast('Video project created. Starting productionÃ¢â‚¬Â¦');
    void runAction('run_to_final','Starting production until it needs your decisionÃ¢â‚¬Â¦');
