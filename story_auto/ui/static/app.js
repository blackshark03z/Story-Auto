...
async function showReview() {
  ...
  try {
    ...
    $('#view').innerHTML = `<section class="surface"><div class="surface-head"><div><h2>Quality review</h2><p>${review.final_path ? `Final duration ${esc(formatDuration(review.duration_seconds))}.` : 'Review flagged scenes before production continues.'}</p></div>${review.final_path ? `<a class="button button-primary" href="${assetUrl(state.project,review.final_path)}" target="_blank" rel="noopener">Open final video</a>` : ''}</div>${qualityCards(review.quality)}</section>
      ${manualBatch}${review.final_path ? `<section class="surface"><h2>Publishing package</h2><p><strong>${esc(publishing.selected_title || review.title)}</strong></p><p class="publishing-copy">${esc(publishing.description || 'Publishing copy has not been prepared yet.')}</p>${publishing.description ? '<button id="copyPublishing" type="button">Copy title & description</button>' : ''}</section>` : ''}
...
async function showSettings() {
  ...
  const dolaDetails = `<section class="settings-section"><p class="eyebrow">EXPERIMENTAL VIDEO PROVIDER</p><h2>Dola accounts</h2><p>Saving cookies keeps the account record, but does not prove sign-in or enable video generation. A dedicated Dola browser profile must pass verification first.</p><dl class="summary-list"><div class="summary-row"><dt>Status</dt><dd>${esc(dola.configured ? 'Saved, not verified' : 'Not configured')}</dd></div><div class="summary-row"><dt>Saved accounts</dt><dd>${esc(dola.account_count || 0)}</dd></div><div class="summary-row"><dt>Connection check</dt><dd>${esc(dola.live_verified ? 'Verified' : 'Not verified')}</dd></div></dl><div class="provider-list" style="margin-top:14px">${dolaRows}</div><div class="field" style="margin-top:16px"><label for="dolaAccountName">Account name for Cookie-Editor export</label><input id="dolaAccountName" type="text" value="dola-main" autocomplete="off" autocapitalize="off" spellcheck="false"><small>Used when you paste the raw JSON export below. Preview shows whether this name is new or will be updated.</small></div><div class="field" style="margin-top:16px"><label for="dolaAccountsInput">Dola cookies</label><textarea id="dolaAccountsInput" rows="5" autocomplete="off" spellcheck="false" placeholder="Paste Cookie-Editor JSON from your signed-in Dola tab"></textarea><small>Paste the full Cookie-Editor JSON array directly. A valid sessionid or sessionid_ss cookie for www.dola.com is required. Advanced: named headers as account-name[TAB]cookie header, one per line, or JSON [{"account_id":"name","cookie":"full cookie"}]. Cookies are encrypted for this Windows user and never shown again.</small></div><div id="dolaPreview" class="hint" aria-live="polite"></div><div class="button-row" style="margin-top:14px"><button id="previewDolaAccounts" type="button">Preview changes</button><button id="saveDolaAccounts" class="button-primary" type="button">Save accounts</button></div></section>`;
