const state = {
  ...
  wizard: { step: 1, content: '', info: null, voice: 'bm_george', style: 'natural', mode: 'hybrid_hook', execution: 'FULL', importedAudio: null, ambientStyle: 'quiet_verdict', fullImage: {image_duration_seconds:30, cadence:'SEMANTIC_ADAPTIVE', motion:'AUTO_CONTINUOUS_ZOOM', audio_visualizer:true}, creating: false }
};
...
async function openWizard() {
  ...
  if (!state.wizard.content) {
    ...
    state.wizard.execution = 'FULL'; state.wizard.importedAudio = null;
...
async function advanceWizard() {
  ...
  try {
    ...
    const created = await api('/api/projects',{method:'POST',body:JSON.stringify({render_mode:wizard.mode,ambient_style:wizard.mode === 'ambient_story' ? wizard.ambientStyle : null,content:wizard.content,settings,imported_audio:wizard.execution === 'FULL' ? null : wizard.importedAudio})});
    ...
    closeWizard(); state.wizard = {step:1,content:'',info:null,voice:wizard.voice,style:'natural',mode:wizard.mode,execution:'FULL',importedAudio:null,ambientStyle:wizard.ambientStyle,fullImage:wizard.fullImage,creating:false}; toast('Video project created.');
