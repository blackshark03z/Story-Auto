...
function humanMode(mode) { return mode === 'full_video_ai' ? 'Full Video (unavailable)' : mode === 'ambient_story' ? 'Ambient Story' : mode === 'full_image' ? 'Full Image' : 'Intro Video + Images'; }
...
function hydrateDraftDefaults(draft, payload) {
  ...
  if (!draft.touched.mode) draft.mode = ['hybrid_hook','full_image'].includes(defaults.render_mode) ? defaults.render_mode : 'hybrid_hook';
...
function renderWizard() {
  ...
  if (wizard.step === 1 || wizard.step === 2) {
    ...
    const qualityPolicy = `<fieldset class="field contextual-field"><legend>Quality review</legend><div class="choice-grid"><label class="choice"><input type="radio" name="qcPolicy" value="AUTO_ACCEPT" ${wizard.qcPolicy === 'AUTO_ACCEPT' ? 'checked' : ''}><strong>Automatic</strong><small>Story Auto continues after technical and provenance checks pass.</small></label><label class="choice"><input type="radio" name="qcPolicy" value="MANUAL_REVIEW" ${wizard.qcPolicy === 'MANUAL_REVIEW' ? 'checked' : ''}><strong>Manual</strong><small>Pause for your review before rendering.</small></label></div></fieldset>`;
    $('#wizardContent').innerHTML = `<fieldset class="field"><legend>Format</legend><div class="choice-grid format-grid"><label class="choice"><input id="formatChoice" type="radio" name="format" value="hybrid_hook" ${wizard.mode === 'hybrid_hook' ? 'checked' : ''}><strong>Intro Video + Images</strong><small>Video-led opening with an image-led story body.</small></label><label class="choice"><input type="radio" name="format" value="full_image" ${wizard.mode === 'full_image' ? 'checked' : ''}><strong>FULL IMAGE</strong><small>Images only, AUTO CONTINUOUS ZOOM, and optional waveform.</small></label><label class="choice" aria-disabled="true"><input type="radio" name="format" value="full_video_ai" disabled><strong>Full Video — Coming soon</strong><small>Full Video is not available in this release.</small></label></div></fieldset>${qualityPolicy}${contextualStyle}${sourceContext}`;
