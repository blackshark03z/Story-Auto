...
function humanMode(mode) { return mode === 'full_video_ai' ? 'Full Video (Coming soon)' : mode === 'ambient_story' ? 'Ambient Story (deferred)' : mode === 'full_image' ? 'Full Image' : 'Intro Video + Images (Coming soon)'; }
...
function renderProject() {
  ...
  ${blocker ? `<section class="attention-card" aria-labelledby="blockerTitle"><div><h2 id="blockerTitle">${blocker.reason_code === 'STUCK_PENDING' ? 'Flow generation appears stuck' : 'Action needed'}</h2><p>${esc(blocker.human_message)}</p><p class="reassurance">Your completed work is saved.</p></div>${blocker.reason_code === 'STUCK_PENDING' ? '<div class="button-row"><button class="button-primary" data-project-action="recheck_flow_generation" type="button">Recheck status</button><button data-project-action="open_flow_project" type="button">Open Flow project</button></div>' : primary}${flow?.status === 'PROJECT_MISMATCH' ? '<button data-rebind-flow type="button">Rebind this Story Auto project</button>' : ''}</section>` : ''}
...
async function advanceWizard() {
  ...
  if (wizard.step === 3) {
    if (wizard.mode !== 'full_image') { showWizardError('Only Full Image is available in this release.','formatChoice'); return; }
