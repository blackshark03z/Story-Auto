...
| New Story Auto direct-cookie canary | Exact isolated attempt `4832cbfc770746c8a8bb24eeda791efc`: one submit using the freshly authenticated saved alias, HTTP 200 SSE but `NO_ACK`, no task ID or asset. Read-only lookup over 10 recent conversations found no exact local request ID; this is not no-effect proof. `evidence/dola-profile-canary-20260923/readonly-reconcile-01.json` | AMBIGUOUS; no retry |
| External browser-UI generation | Gemini/Patchright scratch run injected a session cookie into persistent `acc1` context and submitted via Dola UI. After-send/final screenshots show the same sports-car prompt and completed video card. `D:\Video\AutoVideoPipeline\output\dola_new_generated.mp4` exists: SHA-256 `314c62b5f81ef5075ddb131f83f1e964f09efe04e0ae26b931ee26d9d8945272`, HEVC 1248x704/24 fps, 5.041667 s, 1,067,527 bytes. This proves one external UI path worked, not the Story Auto adapter or repeated stability; the scratch script contains a plaintext credential and must not be copied | EXTERNAL LIVE UI PASS ONCE; PRODUCT HOLD |
| Story Auto browser-UI candidate | Isolated clone/branch has an opt-in Patchright client, one-action send, durable native request ID, exact read-only conversation/input receipt, and unchanged poll/download attribution. Focused offline tests pass. Its dedicated profile passed three restart preflights (10s/5s/10s) after fixing UI hydration; every run stopped at an empty composer with zero generation. A separate one-shot canary project has zero attempts. Normal UI submission stays disabled; this transport has not submitted live | PRE-SUBMIT PASS; LIVE/PRODUCT HOLD |
