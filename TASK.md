...
fourth-canary runtime, UI preflight passed for O2 and the review dialog showed
the exact 5s slot, account and prompt. One click created attempt
`fd04bae72e0b47879243321df8847738` and confirmed conversation
`38417989246554641`, one native input and one provider submission. Dola's
linked reply asked whether to change the duration to 15s; there was no video.
The Owner did not approve 15s and authorized one **new** 5s request. The O2
attempt remains `OPERATOR_DECISION_REQUIRED`, with its quota effect unverified;
it was not erased, retried or answered. Poll now recognizes this exact linked
question and the UI shows “NEEDS YOUR DECISION” instead of indefinitely
showing GENERATING. Focused code tests include same-conversation linkage and
zero redispatch on recovery.

A distinct product UI project then used a prompt explicitly saying
“five-second”. Its UI preflight, review and one Create click produced attempt
`463d435bdb1742f18e2ed33b9d6018e4`, confirmed conversation
`38417972965476625`, `submit_attempts=1`, `provider_submissions=1`, and
`SUCCEEDED`. The same-conversation Base64-linked `master` was acquired and
canonically imported into OPENING_O1 READY: source SHA256
`4619ef31e9355956513cb078d355a954233bbf33169bca35476d55d81ea0b314`,
normalized SHA256 `635358a3e1d401557cc55b0c784c12f93ee481bc8db104986235472118431820`.
The 2s source frame was visually inspected and has no Dola AI logo. The UI
displayed Generated by Dola and READY. In-app browser video playback crashed
its own tab; independent file validation and the earlier Chrome playback of
the first Dola master remain separate evidence, not a claim that this second
clip played successfully in the in-app browser. O2/O3 of the new project have
no attempts. Evidence and authorization records are in
`D:\Story Auto\evidence\dola-product-ui-review-20260923`.

This closes one product-button create/receipt/result/import journey, with
one provider response requiring human decision in a separate attempt. It does
not compose a new final video or establish repeatable unattended Dola success.
No further generation is authorized by these two Owner choices.

## 2026-09-23 product acceptance continuation after Dola live success

The exact candidate checkout also served the already Owner-accepted Flow final
from `D:\Story Auto\evidence\flow-real-product-journey-20260921` at local port
8783. SHA256 remained `de1679ad0307e13ad8fd5a240b62af3dbb39d654a17a6a1b9559c44852acd132`.
At 1440px and 576px, Chrome reached the project result, found one Open final
video action, played the inline 1280x720 video and sought to about 35.5s with
readyState4, no video error and no page error. Screenshots:
`candidate-final-1440-20260923.png`, `candidate-final-576-20260923.png` in that
evidence root. This associates the accepted final with the current candidate
UI code; it does not imply that the Dola canary is part of that final.

Gemini's additional master URL suggestion was checked against the fourth
conversation. The exact linked creation has a Base64 `video_model.video_list`
main URL. A read-only same-conversation fetch and one media download produced
a 521034-byte, 1280x720, 5.041667-second master with SHA256
`f1a6d3e8f514c40b61a1b13fbd2906b656acd6b477345876530413b07f059233`.
Frames at 2s/4s showed no Dola AI watermark. The original canary manifest was
not rewritten. The UI-review copy imported this linked master as O1 revision2
under the same provider/task/account identity and played it in Chrome. Client
poll now prefers a safe linked master; otherwise it retains the proven preview
URL. Independent review identified and we fixed pre-send CAPTCHA closure and
preview-file reuse under a master label: headed pre-send challenges now wait
for the human and recheck controls; downloads use variant-specific paths plus
persisted hash before reuse. A further fail-closed composer guard rejects any
stale draft before the initial send and only reuses an exact retained prompt
after a human CAPTCHA check. Independent narrow review found no remaining
blocker in that guard. Focused Dola tests: 69 passed, 32 subtests.
See `docs/DOLA_BROWSER_UI_RUNBOOK_20260923.md` and the archived Gemini
paste for exact distinctions. No new generation or quota use occurred.

The gated product path now constructs a visible browser runner so a human can
handle a CAPTCHA in the same window. Focused Dola tests: 41 passed and 15
subtests passed. A copy of the fourth canary runtime at
`D:\Story Auto\evidence\dola-product-ui-review-20260923` enabled Hybrid CUJ
for rendered inspection without altering the original live evidence. The UI
showed the real O1 as Generated by Dola, and Chrome played its exact normalized
video at 1440px and 576px for about 1.5 seconds with 1280x720 metadata and
zero page/video errors. UI Check Dola session succeeded for O2, while the
manifest still has no O2/O3 attempt. No new Dola generation was sent.
Current scope and remaining release gates are in
`docs/COOKIE_FLOW_PRODUCT_ACCEPTANCE_PLAN.md`.

## 2026-09-23 fourth Dola canary: video acquired successfully

Runbook/SoT for reproduction and Gemini comparison:
[`docs/DOLA_BROWSER_UI_RUNBOOK_20260923.md`](docs/DOLA_BROWSER_UI_RUNBOOK_20260923.md).
Includes the archived original Gemini blueprint, actual successful procedure,
source/runtime identity, evidence paths, and confirmed versus unverified claims.

Owner explicitly authorized one further send in the async reply. Fourth canary
reuses the third account-bound profile, preserves cookies, and runs headed.
The runner now leaves a visible CAPTCHA open for manual verification and
excludes that human wait from the observation deadline; it never resends.
Focused verification: 35 tests and 15 subtests passed before this live run.
Preflight passed UI_READY_NO_SUBMIT. The single send created attempt
`fb732e3e25af414a8d8f4628f16187a3`; exact native-input readback confirmed
conversation `38417972940811793` at 2026-09-23T10:14:09Z.
`submit_attempts=1`, `provider_submissions=1`, `dispatch_state=CONFIRMED`.
Evidence: `D:\Story Auto\evidence\dola-browser-ui-canary-4-20260923`.
The authorization is consumed. Await/poll only this receipt; no new dispatch.
This supersedes the previous pending manual-window step, not the preserved
ambiguous status of the first three attempts.

Completed at 2026-09-23T10:17:34Z: same-receipt recovery returned SUCCEEDED,
slot OPENING_O1 READY. Source video is 1280x720, 24fps, 5.041667 seconds;
normalized video is 1280x720, 24fps, 5 seconds, audio stripped.
Source SHA256: c4a0761ba7482184a6f4dfc547efac1f7289b3b485ab626c65e6f4f9ec396ccc.
Normalized SHA256: fe0dd262ccfcbc94efd23d4efa9f47b98fabfaa1a7b6467c40d014609a9fa243.
Final browser screenshot shows the exact lighthouse prompt as a sent message,
empty composer, Dola generation acknowledgement and 2 credits left today.
No CAPTCHA was observed during this attempt. One live acquisition is proven;
unattended reliability and Owner quality acceptance are not established.

## 2026-09-23 preserve bound Dola profile and manual verification window

Owner requested preserving profile state and opening a visible browser for
manual verification. Both preflight and run now seed only missing Dola cookies;
they do not clear cookies or overwrite existing cookies with the saved export.
Profile binding/account rotation remains enforced. Focused tests: 34 passed,
15 subtests passed. `tools/dola_browser_ui_operator.py` opened the exact third
canary's hash-bound profile in a headed browser and waits for the operator to
close it. The helper performs no prompt entry, submit, CAPTCHA solving, or
automatic retry. No anti-detection flags were added. Opening the window does
not establish completed verification or future CAPTCHA exemption. The current
challenge may not reappear after the prior session closed; do not send a new
generation merely to trigger it. Operator verification is pending.

## 2026-09-23 third Dola canary: post-send CAPTCHA observed

Owner explicitly requested another try. A distinct third project used one
submit action with focused keyboard typing and Enter, a 5-second yellow-sailboat
prompt, and 180-second observation. Preflight initially missed a loading model
control; read-only recheck passed, and the model wait was bounded to 10 seconds.
Attempt `483f94298e544666b794e4ad265117ec` remains AMBIGUOUS, one submit attempt,
zero confirmed receipts. The final screenshot shows the prompt as a sent chat
message with a red error icon, the composer empty, and a "Verify to continue"
puzzle CAPTCHA. HTTP 200 on completion is not job acceptance. The immediate
after-send screenshot was too early to distinguish input from sent-message
state; the final screenshot establishes that the UI processed the send and
then presented an operator verification challenge. No video or confirmed job
is evidenced. Do not solve or bypass the CAPTCHA automatically, or redispatch.
Evidence root: `D:\Story Auto\evidence\dola-browser-ui-canary-3-20260923`,
including `PRE_DISPATCH.json` and `screenshots/final.png`. The current runner
does not yet classify this post-send challenge promptly and instead times out
as DOLA_UI_RECEIPT_MISSING. This observation does not establish the cause of
the earlier two attempts. No additional send followed this challenge.
...
The confirmation's text, account scope, rate/quota conditions, and output-use
terms are not independently recorded here; this is an Owner attestation, not
documentary vendor clearance. After disclosure that the previous ambiguous
request might already have created a job or consumed a credit, the Owner
explicitly authorized at most **one new, separate** Dola test request. Keep
the old attempt frozen; use a different isolated project/slot/attempt, no
automatic retry or account rotation, and stop on an uncertain response.
Before dispatch, verify the saved session read-only and finish status-only
HTTP diagnostics. This does not yet qualify Dola or the whole product.

## 2026-09-23 historical Dola provider permission HOLD
...
At this checkpoint no vendor permission or official Dola API contract was
recorded in the project. This is not a legal determination. At that time do not
