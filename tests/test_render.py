...
class RenderServiceRecoveryTests(unittest.TestCase):
    def test_checkpoint_resume_rebuilds_only_missing_render_descendants(self) -> None:
        with tempfile.TemporaryDirectory() as root:
            ...
            forced = run_render_stages(runtime.root, config.project_id, force_final=True)
            self.assertEqual(forced["actions"]["final_render"], "RUN")
            self.assertTrue(all(action == "SKIP" for action in forced["actions"]["clips"].values()))
    ...
    def test_full_video_multi_part_render_resume_and_render_only_invalidation(self) -> None:
        with tempfile.TemporaryDirectory() as root:
            ...
            final_manifest=first["final_manifest"]
            self.assertEqual(read_json(render_plan_path)["render_mode"],"full_video_ai")
            self.assertEqual(final_manifest["render_plan_sha256"],sha256_file(render_plan_path))
            self.assertEqual(read_json(paths.artifact_path("output/audio_plan.json"))["source_video_audio"],"MUTE")
            self.assertEqual(len(final_manifest["streams"]["audio"]),1)
            self.assertAlmostEqual(final_manifest["duration_seconds"],2.0,delta=.12)
            self.assertEqual(final_manifest["final_sha256"],sha256_file(paths.artifact_path("output/final.mp4")))
