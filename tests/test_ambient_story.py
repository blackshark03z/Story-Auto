...
class AmbientProjectAndPlanningTests(unittest.TestCase):
    ...
    def test_visual_chapters_budgets_image_only_resume_and_narrow_invalidation(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            self.assertEqual(len(quiet_shots["shots"]), 5)
            self.assertEqual(quiet_shots["asset_budget"], {"preferred_min":2,"preferred_max":5,"planned_images":5,"budget_exception_reason":None})
            ...
            self.assertEqual(len(hidden_shots["shots"]), 7)
